using PSW.Shared.Constants;

namespace PSW.Shared.Constants;

public static class DefaultValues
{
    public const string TemplateName = GlobalConstants.TEMPLATE_NAME_UMBRACO;
    public const string StarterKitPackage = "clean";
    public const string ProjectName = "MyProject";
    public const string SolutionName = "MySolution";
    public const string DatabaseType = "SQLite";
    public const string ConnectionString = @"server=(local)\SQLEXPRESS;database=myDatabase;user id=myUser;password='myPassword';TrustServerCertificate=true;";
    public const string UserFriendlyName = "Administrator";
    public const string UserEmail = "admin@example.com";
    public const string UserPassword = "1234567890!";
}